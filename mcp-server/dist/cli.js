...
async function followupGapInputs(options) {
    const jobNumbers = [];
    if (options.input) {
    ...
    if (jobNumbers.length === 0)
        throw new Error("Provide --input <csv> or --job-numbers <comma-separated values>.");
    return jobNumbers;
}
function followupGapConcurrency(value) {
    if (!/^\d+$/.test(value))
...
addReadOptions(reports
    ...
    .action((options) => runRead("Follow-up gaps", options, async (client) => {
