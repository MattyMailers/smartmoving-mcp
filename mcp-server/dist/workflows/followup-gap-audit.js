export function normalizeJobOrQuoteNumber(value) {
    const input = value.trim();
    if (!input)
        return { input, error: "Job or quote number is blank." };
    if (input.length > 100)
        return { input, error: "Job or quote number must not exceed 100 characters." };
    const candidate = input.replace(/^(?:job(?:\s+number)?|quote)\s*#?\s*/i, "").trim();
    const qPrefixed = /^Q-\d+(?:-\d+)*$/i.exec(candidate);
    if (qPrefixed)
        return { input, quoteNumber: candidate.toUpperCase() };
    const numeric = /^(\d+)(?:-\d+)?$/.exec(candidate);
    if (!numeric)
        return { input, error: "Unrecognized SmartMoving job or quote number." };
...
function completedFollowup(value) {
...
function assignedFollowup(value) {
    if (!value || typeof value !== "object")
        return false;
    const assignedToId = value.assignedToId;
    return typeof assignedToId === "string" && assignedToId.trim().length > 0;
}
export function classifyFollowups(value) {
    ...
    const classification = list.length === 0
        ? "no_followups"
...
function isOpportunityNotFound(error) {
    const statusCode = statusCodeOf(error);
    if (statusCode === 404)
        return true;
    if (statusCode !== 400 || !error || typeof error !== "object")
        return false;
...
async function getWithRetry(client, path, retryDelayMs) {
...
async function auditQuote(client, quoteNumber, retryDelayMs) {
...
export async function runFollowupGapAudit(client, options) {
    if (!Array.isArray(options.jobNumbers) || options.jobNumbers.length === 0)
        throw new Error("At least one job or quote number is required.");
    if (options.jobNumbers.length > 1000)
        throw new Error("A maximum of 1000 job or quote numbers can be audited at once.");
    ...
    if (!Number.isFinite(retryDelayMs) || retryDelayMs < 0 || retryDelayMs > 60_000)
        throw new Error("Retry delay must be between 0 and 60000 milliseconds.");
    ...
    const byQuote = new Map(quoteResults.map((row) => [row.quoteNumber, row]));
    const rows = normalized.map((item) => "error" in item
    ...
    const missingClassifications = new Set(["no_followups", "completed_only", "open_unassigned"]);
...
//# sourceMappingURL=followup-gap-audit.js.map