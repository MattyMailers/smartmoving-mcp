...
function printSmoke(result, options) {
    if (options.json) {
...
function optionsWithGlobals(command, options) {
    const globals = command.optsWithGlobals();
    return { ...options, json: options.json === true || globals.json === true };
}
async function runReadSmoke(options) {
    const checks = [];
    try {
...
export function registerSmokeCommand(program, helpers) {
    const smoke = program
        .command("smoke")
