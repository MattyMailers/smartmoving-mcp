...
export function registerLeadTools(server, client) {
    // ---------- list_leads ----------
    server.tool("list_leads", "List leads with pagination. Returns a paginated list of lead records. Leads are prospective customers who have not yet been converted to opportunities.", {
    ...
    // ---------- get_lead ----------
    server.tool("get_lead", "Get detailed information about a specific lead by ID. Returns all lead details including contact info, move details, origin/destination addresses, and current status.", {
    ...
    // ---------- create_lead ----------
    server.tool("create_lead", "Create a new lead in SmartMoving. Premium tier endpoint. A lead represents a potential customer inquiry. You can provide either separate firstName/lastName fields or a combined 'name' field. The referralSourceId is required - use get_referral_sources to find valid IDs.", {
    ...
    // ---------- update_lead ----------
    server.tool("update_lead", "Fully update an existing lead record. Premium tier endpoint. This is a PUT operation - all fields will be set to the provided values (omitted fields may be cleared). For partial updates, use patch_lead instead.", {
    ...
    // ---------- patch_lead ----------
    server.tool("patch_lead", "Partially update an existing lead. Premium tier endpoint. Only the fields you provide will be modified; all other fields remain unchanged. Use this for small updates to a lead.", {
    ...
    server.tool("get_leads_by_salesperson", "List leads assigned to a specific salesperson. Premium tier endpoint. Useful for viewing a sales rep's pipeline of uncontacted or in-progress leads.", {
    ...
    server.tool("convert_lead_to_opportunity", "Convert a lead into an opportunity. Premium tier endpoint. This transitions a lead from the initial inquiry stage into a full opportunity/quote that can be worked, priced, and booked. The lead record is consumed and an opportunity is created.", {
    ...
    // ---------- get_lead_statuses ----------
    server.tool("get_lead_statuses", "Get all possible lead status values. Returns the list of statuses a lead can be in (e.g. New, Contacted, Qualified, Lost, BadLead, Converted). Useful for understanding lead status codes.", {}, async () => {
        try {
