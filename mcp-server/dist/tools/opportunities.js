...
export function registerOpportunityTools(server, client) {
    ...
    server.tool("get_opportunity_by_quote", "Look up an opportunity by its quote number (e.g. 'Q-12345'). Use this when you have a quote number but not the opportunity UUID. Returns the same detailed view as get_opportunity.", {
    ...
    server.tool("get_opportunity_audit", "Get the audit trail / activity log for an opportunity. Shows a chronological history of all changes, status transitions, and actions performed on the opportunity.", {
    ...
    server.tool("get_opportunity_documents", "List all documents attached to an opportunity. Premium tier endpoint. Returns file metadata including name, category, URL, and upload date.", {
        opportunityId: z.string().uuid().describe("The opportunity ID"),
    }, async (params) => {
        try {
    ...
    server.tool("get_opportunity_payments", "List all payments recorded for an opportunity. Returns payment details including type, amount, date, and reference numbers.", {
        opportunityId: z.string().uuid().describe("The opportunity ID"),
    }, async (params) => {
        try {
    ...
    // ---------- add_attachment ----------
    server.tool("add_attachment", "Upload a file attachment to an opportunity. Premium tier endpoint. The file must be provided as a base64-encoded string. Use fileCategory to classify the document type.", {
    ...
    // ---------- create_rooms ----------
    server.tool("create_rooms", "Create rooms for an opportunity's inventory. Premium tier endpoint. Rooms are used to organize inventory items (e.g. 'Living Room', 'Master Bedroom'). Use get_room_types to find valid room type IDs.", {
        opportunityId: z.string().uuid().describe("The opportunity ID"),
