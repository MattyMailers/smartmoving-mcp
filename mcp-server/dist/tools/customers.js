...
export function registerCustomerTools(server, client) {
    // ---------- list_customers ----------
    server.tool("list_customers", "List customers with pagination. Returns a paginated list of customer records from SmartMoving. Use this to browse or iterate through all customers in the system. Supports filtering by service date range.", {
    ...
    // ---------- get_customer ----------
    server.tool("get_customer", "Get detailed information about a specific customer by their ID. Returns full customer profile including contact info, address, notes, and dates.", {
    ...
    // ---------- search_customers ----------
    server.tool("search_customers", "Search customers by name, phone, or email. Premium tier endpoint. The search query must be at least 3 characters. Use this when you need to find a customer by partial information rather than browsing the full list.", {
    ...
    // ---------- create_customer ----------
    server.tool("create_customer", "Create a new customer record in SmartMoving. Premium tier endpoint. At minimum a name (first/last or company) should be provided. Returns the created customer with its new ID.", {
    ...
    // ---------- update_customer ----------
    server.tool("update_customer", "Update an existing customer record. Premium tier endpoint. Provide the customer ID and any fields you want to change. Fields not included will remain unchanged.", {
    ...
    server.tool("get_customer_opportunities", "List all opportunities (quotes/moves) associated with a specific customer. Returns an array of opportunity summaries for the given customer ID.", {
    ...
    server.tool("get_customer_storage_accounts", "List storage accounts for a specific customer. Returns storage unit details, monthly rates, and account status for the customer.", {
        customerId: z.string().uuid().describe("The unique customer ID"),
    }, async (params) => {
        try {
    ...
    server.tool("get_customer_service_tickets", "List service tickets for a specific customer. Premium tier endpoint. Returns any open or resolved service tickets (claims, complaints, etc.) for the customer.", {
        customerId: z.string().uuid().describe("The unique customer ID"),
    }, async (params) => {
        try {
