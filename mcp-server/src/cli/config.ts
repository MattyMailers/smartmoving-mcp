...
export interface ResolvedProfileAuth {
  profile: SmartMovingProfileConfig & { name: string; apiKeySource: ApiKeySource };
  apiKey?: string;
  apiKeyDetail: string;
  baseUrl: string;
}
...
export function assertSafeEnvName(value: string): string {
...
export function normalizeApiKeySource(value: ApiKeySource | undefined): ApiKeySource {
  return value ?? "env";
}

export async function readConfig(path = configPath()): Promise<SmartMovingCliConfig> {
  const raw = await readFile(path, "utf8");
  const parsed = JSON.parse(raw) as SmartMovingCliConfig;
...
export async function writeInitialConfig(options: InitConfigOptions, path = configPath()): Promise<SmartMovingCliConfig> {
  ...
  const config: SmartMovingCliConfig = {
...
export function selectProfile(config: SmartMovingCliConfig, profileName?: string): SmartMovingProfileConfig & { name: string; apiKeySource: ApiKeySource } {
...
export async function readCredentials(path = credentialsPath()): Promise<SmartMovingCredentials> {
  const raw = await readFile(path, "utf8");
  const parsed = JSON.parse(raw) as SmartMovingCredentials;
...
export async function writeStoredApiKey(profile: string, apiKey: string, path = credentialsPath()): Promise<SmartMovingCredentials> {
  ...
  let credentials: SmartMovingCredentials = { version: CREDENTIALS_VERSION, profiles: {} };
  try {
...
export async function readStoredApiKey(profile: string, path = credentialsPath()): Promise<string | undefined> {
  const credentials = await readCredentials(path);
  return credentials.profiles[profile]?.apiKey;
}

export async function resolveProfileAuth(profileName?: string): Promise<ResolvedProfileAuth> {
