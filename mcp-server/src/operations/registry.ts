import { z } from "zod";
import type { OperationDefinition, OperationGroup, OperationSafety, OperationSchemaContract, OperationSchemaEntry } from "./types.js";

const notImplementedHandler: OperationDefinition["handler"] = async () => {
...
export const operationRegistry: OperationDefinition[] = operationMetadata.map((operation) => ({
  ...operation,
...
export function operationSchemaContract(filters: OperationSchemaFilters = {}): OperationSchemaContract {
  const operations: OperationSchemaEntry[] = operationRegistry
