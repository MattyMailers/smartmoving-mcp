...
function parseCsv(text: string): string[][] {
  ...
  let field = "";
  let quoted = false;
  const finishField = (): void => { row.push(field); field = ""; };
...
export function jobNumbersFromCsv(text: string, requestedColumn?: string): string[] {
  ...
  for (const candidate of candidates) {
    ...
    if (matches.length > 1) throw new Error(`Column '${headers[matches[0]]}' appears more than once.`);
    if (matches.length === 1) { columnIndex = matches[0]; break; }
  }
  if (columnIndex < 0) {
  ...
  if (values.length === 0) throw new Error(`CSV column '${headers[columnIndex]}' contains no job or quote numbers.`);
  return values;
}
