...
export function registerCommunicationTools(server: McpServer, client: SmartMovingClient): void {
  // ---------- log_call ----------
  server.tool(
    "log_call",
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const result = await client.post(
          `/api/premium/opportunities/${params.opportunityId}/communication/notes`,
          { notes: params.notes },
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
