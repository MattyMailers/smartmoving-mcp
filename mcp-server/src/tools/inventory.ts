...
export function registerInventoryTools(server: McpServer, client: SmartMovingClient): void {
    ...
    async (params) => {
      try {
        ...
        const result = await client.put(
          `/api/premium/opportunities/${params.opportunityId}/inventory/rooms/${params.roomId}/items/${params.inventoryItemId}`,
          body,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const result = await client.post(
          `/api/premium/opportunities/${params.opportunityId}/inventory/submit`,
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (error) {
        return { content: [{ type: "text", text: `Error: ${(error as Error).message ?? JSON.stringify(error)}` }], isError: true };
